...
pulp-api() { bash "${DEVCONTAINER_DEV_SCRIPTS}/runtime/run-api.sh"; }
pulp-content() { bash "${DEVCONTAINER_DEV_SCRIPTS}/runtime/run-content.sh"; }
pulp-worker() { pulpcore-worker; }

pulp-services() {
  export PYTHONSAFEPATH=1
  concurrently --names "api,content,worker" --prefix-colors "green,blue,magenta" \
    "env PYTHONSAFEPATH=1 bash ${DEVCONTAINER_DEV_SCRIPTS}/runtime/run-api.sh" \
    "env PYTHONSAFEPATH=1 bash ${DEVCONTAINER_DEV_SCRIPTS}/runtime/run-content.sh" \
