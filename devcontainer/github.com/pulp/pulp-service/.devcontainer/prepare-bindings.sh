...
# Service variant of prepare-bindings: domainEnabled=true (pulp-service settings)
# and package naming aligned with pulp-service's pulp-test helper.
...
if [ ${#components[@]} -eq 0 ]; then
    ...
    echo "Example: prepare-bindings.sh core maven python rpm npm service"
    exit 1
fi

################################################################################
# Ensure OpenAPI Generator CLI and templates are installed
################################################################################
...
################################################################################
# Generate and install bindings for each component
################################################################################

for comp in "${components[@]}"; do
    # The core component uses "pulpcore" as its Python package name (matching the PyPI
    # pulpcore-client package that tests import from). All other components use "pulp_<comp>".
    if [ "$comp" = "core" ]; then
        python_pkg="pulpcore"
