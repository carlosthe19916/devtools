...
uv pip install -e /workspace/pulp_service
...
uv pip install pulp-file
uv pip install httpie pulp-cli packaging setproctitle
...
uv pip install "pytest<8" pytest-django gnupg
if curl -sf -o /tmp/functest_requirements.txt \
  ...
  uv pip install -r /tmp/functest_requirements.txt || true
fi
if curl -sf -o /tmp/unittest_requirements.txt \
  ...
  uv pip install -r /tmp/unittest_requirements.txt || true
fi
