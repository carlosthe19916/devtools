...
#   patches.sh reapply
#
# Default tree is /repositories/pulpcore (mounted pulpcore).
set -euo pipefail
...
usage() {
  ...
  echo "  apply [tree]       Apply all patches (default tree: /repositories/pulpcore)"
  echo "  remove             Reverse all patches from /repositories/pulpcore"
  echo "  remove <name>      Reverse one patch (e.g. 0044-....patch)"
  echo "  reapply            Reverse all, then apply to /repositories/pulpcore"
  exit 1
}
