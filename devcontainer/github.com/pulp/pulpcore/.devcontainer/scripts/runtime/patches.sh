...
#   patches.sh reapply
#
# Default tree is /workspace (editable pulpcore).
set -euo pipefail
...
usage() {
  ...
  echo "  apply [tree]       Apply all patches (default tree: /workspace)"
  echo "  remove             Reverse all patches from /workspace"
  echo "  remove <name>      Reverse one patch (e.g. 0044-....patch)"
  echo "  reapply            Reverse all, then apply to /workspace"
  exit 1
}
