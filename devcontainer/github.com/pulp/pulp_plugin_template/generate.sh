...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Note: pulpcore's .devcontainer is hand-maintained at"
    echo "  ../pulpcore/.devcontainer/ (not generated from this template)."
    exit 1
fi

for PLUGIN in "$@"; do
    if [ "$PLUGIN" = "core" ]; then
        echo "ERROR: pulpcore is no longer generated from this template." >&2
        echo "Edit ../pulpcore/.devcontainer/ directly." >&2
        exit 1
    fi
